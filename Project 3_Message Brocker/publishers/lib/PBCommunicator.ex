defmodule PBCommunicator do
    use GenServer
    require Logger

    def init([port]) do
        :ets.new(:coin_data, [:set, :public, :named_table])
        {:ok, port}
    end
    ...
    def handle_info({:tcp,socket,packet}, state) do
        ...
        cond do
                message["request"] == "QoS2 publish" -> send(:publisher2,{:PUBREC,message["response"]})
    ...
    def handle_info({:send,data}, state) do
        :ets.insert(:coin_data,{data[:id],data})
